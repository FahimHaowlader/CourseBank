...
const UpdateCourseInfo = () => {
  ...
  const { infoModal, setInfoModal, course, setCourse } = useCourse();
  ...
  // FIX: Added optional check for 'e' so it works for both form submit and Retry button
  const handleUpdateInfo = async (e) => {
    ...
    setLoading(true); 
    try {
      const res = await PrivateApi.patch(`/update-basic-info/${course._id}`, { updatedData: data });
      if (res.data.success) {
        setCourse((prev) => ({
          ...prev,
          ...data 
    ...
    <>
      ...
      {infoModal.openModal && infoModal.status === "error" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" role="dialog">
          <div className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={() => !loading && setInfoModal({})}></div>
          <div className="relative w-full max-w-2xl transform overflow-hidden rounded-3xl bg-white dark:bg-card-dark p-10 sm:p-14 text-center shadow-2xl border border-border-light dark:border-border-dark">
            <div className="flex flex-col items-center gap-8">
              ...
              <div className="space-y-1">
                <h3 className="text-4xl font-bold text-text-main dark:text-white leading-tight">Update Failed</h3>
              ...
              <div className="flex flex-col sm:flex-row w-full gap-3 sm:gap-6 mt-4">
                <button 
                  disabled={loading}
                  onClick={() => setInfoModal({})}
                  className="flex-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 py-4 text-xl font-semibold text-text-main dark:text-gray-300 hover:bg-gray-50 transition-colors cursor-pointer disabled:opacity-50"
                >
                  Cancel
                </button>
                <button 
                  ...
                  disabled={loading}
                  className={`flex-1 rounded-xl bg-orange-500 py-4 text-xl font-semibold text-white shadow-sm transition-all flex items-center justify-center min-h-[64px] ${
                    loading ? "opacity-80 cursor-not-allowed" : "hover:bg-orange-600 active:scale-[0.98] cursor-pointer"
                  }`}
                >
