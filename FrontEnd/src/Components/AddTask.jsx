...
const AddTask = () => {
  ...
  const handleSuccess = () => setTaskModal({ openModal: true, status: "success" });
  const handleRetry = () => setTaskModal({ openModal: true, status: "update" });

  if (!taskModal.openModal) return null;

  // Standardized ModalWrapper matching AddMaterial size and layout
      ...
      {taskModal.status === "update" && (
        <ModalWrapper>
          <div className="px-6 py-5 border-b border-border-light dark:border-border-dark flex justify-between items-center">
            <h3 className="text-lg font-bold text-text-main dark:text-white">Add Task Or Assignment</h3>
          ...
          <div className="p-8 pt-5 space-y-6">
            <label className="flex flex-col gap-1.5 w-full">
              ...
              <div className="relative flex items-center w-full border border-border-light dark:border-border-dark rounded-lg">
                ...
                <input type="text" placeholder="e.g. Midterm Project" className="w-full h-11 pl-10 pr-4 rounded-lg bg-white dark:bg-background-dark border-0 focus:ring-0 text-sm text-text-main dark:text-white" />
              </div>
            </label>
            <label className="flex flex-col gap-1.5 w-full">
          ...
          <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-3 border-t border-border-light dark:border-border-dark">
            ...
            <button onClick={handleSuccess} className="px-5 py-2 rounded-lg bg-primary text-white text-sm font-medium hover:bg-teal-700 transition-colors cursor-pointer">Add Task</button>
          </div>
        </ModalWrapper>
      )}
      ...
      {taskModal.status === "success" && (
        <ModalWrapper>
          <div className="p-10 text-center flex flex-col items-center">
            ...
            <p className="text-xl text-text-secondary dark:text-gray-400 mb-8">Course Task has been added successfully.</p>
